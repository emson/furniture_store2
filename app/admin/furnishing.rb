...
form do |f|
  f.inputs "Details" do
    f.input :furnishing_type, :as => :select, :collection => options_for_select(%w{armchair chair sofa}, default_select(furnishing, :furnishing_type, 'armchair'))
    f.input :shape, :as => :select, :collection => options_for_select(%w{arun avon dart frome hamble lambourn otter parrett stour wylye}, default_select(furnishing, :shape, 'arun'))
